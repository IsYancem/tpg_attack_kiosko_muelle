...
class AuthApiService {
  ...
  // Nuevo método para login de middleware
  static Future<Map<String, dynamic>?> loginMiddleware() async {
    try {
      final username = KioskUserEnv.usuario;
      final password = dotenv.env['PASSWORD_MIDDLEWARE'] ?? '';
      final app = dotenv.env['APP_MIDDLEWARE'] ?? '';

      if (username.isEmpty || password.isEmpty || app.isEmpty) {
        LogService.instance.logError('MiddlewareLogin', {
          'error': 'Credenciales de middleware no configuradas en .env',
        });
        return null;
      }

      final response = await http.post(
        Uri.parse('${_baseUrl}auth/middleware/login'),
        headers: {
          'Content-Type': 'application/json',
      ...
      LogService.instance.logRequest('MiddlewareLogin-Request', {
        'username': username,
        'app': app,
        'url': '${_baseUrl}auth/middleware/login',
      ...
      if (response.statusCode >= 200 && response.statusCode < 300) {
        ...
        LogService.instance.logRequest('MiddlewareLogin-Success', {
          'user': data['user'],
          'has_access_token': data['accessToken'] != null,
          'has_refresh_token': data['refreshToken'] != null,
          'status_code': response.statusCode,
        ...
        LogService.instance.logError('MiddlewareLogin-Failed', {
          'status_code': response.statusCode,
      ...
      LogService.instance.logError('MiddlewareLogin-Exception', {
        'error': e.toString(),
        'stack': st.toString(),
      });
      return null;
