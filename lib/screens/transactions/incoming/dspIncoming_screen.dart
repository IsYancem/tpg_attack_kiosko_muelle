// lib/screens/dsp_incoming/dsp_incoming_screen.dart
// OPTIMIZADO 2025-12-09 - Con timing logs detallados
import 'package:flutter/material.dart';
import 'package:flutter/scheduler.dart';
import 'package:provider/provider.dart';
import 'package:tpg_attack_kiosko_muelle/config/incoming/dsp_incoming_visibility_config.dart';
import 'package:tpg_attack_kiosko_muelle/services/apis/dsp/dsp_transaction_runner.dart';
...
import 'package:tpg_attack_kiosko_muelle/widgets/dsp_incoming/columna1_driver_dsp.dart';
import 'package:tpg_attack_kiosko_muelle/widgets/dsp_incoming/columna2_importador_dsp.dart';
import 'package:tpg_attack_kiosko_muelle/widgets/dsp_incoming/columna3_mapa_dsp.dart';

class DspIncomingScreen extends StatelessWidget {
  const DspIncomingScreen({super.key});

  @override
  Widget build(BuildContext context) {
    return const _DspIncomingScreenBody();
  }
}

class _DspIncomingScreenBody extends StatefulWidget {
  const _DspIncomingScreenBody();

  @override
  State<_DspIncomingScreenBody> createState() => _DspIncomingScreenBodyState();
}

class _DspIncomingScreenBodyState extends State<_DspIncomingScreenBody>
    with TickerProviderStateMixin {
  DspTransactionRunner? _runner;
  ...
  void didChangeDependencies() {
    ...
    if (!_runnerStarted) {
      ...
      _manager!.setManyWithoutNotify({
        'isLoading': true,
        'mensajeInferior': 'Procesando transacción DSP...\nPor favor espere.',
      });
    ...
    final widget = Scaffold(
      ...
      body: Column(
        children: [
          ...
          if (DspIncomingVisibilityConfig.show['header.titulo'] ?? true)
            Selector<AtkTransactionManager, int?>(
              ...
              builder: (context, flowRemaining, _) {
                final headerCountdownSeconds =
                    (DspIncomingVisibilityConfig.show['header.countdown'] ??
                        true)
                    ? (flowRemaining ?? 300)
                    : 0;
                return AtkHeaderTransaction(
                  ...
                  height: hHeader,
                  assetImagePath:
                      DspIncomingVisibilityConfig.show['header.logo'] ?? true
          ...
          if (DspIncomingVisibilityConfig.show['subheader.textoDespacho'] ??
              true)
          ...
          // Body
          if ((DspIncomingVisibilityConfig.show['col1.driver'] ?? true) ||
              (DspIncomingVisibilityConfig.show['col2.importador'] ?? true) ||
              (DspIncomingVisibilityConfig.show['col3.mapa'] ?? true))
            SizedBox(
              height: hBody,
              child: Padding(
                ...
                child: Row(
                  ...
                  children: [
                    if (DspIncomingVisibilityConfig.show['col1.driver'] ?? true)
                      const Expanded(flex: 25, child: Columna1DriverDsp()),
                    if ((DspIncomingVisibilityConfig.show['col1.driver'] ??
                            true) &&
                        ((DspIncomingVisibilityConfig.show['col2.importador'] ??
                                true) ||
                            (DspIncomingVisibilityConfig.show['col3.mapa'] ??
                                true)))
                      const SizedBox(width: 12),
                    if (DspIncomingVisibilityConfig.show['col2.importador'] ??
                        true)
                      const Expanded(flex: 45, child: Columna2ImportadorDsp()),
                    if ((DspIncomingVisibilityConfig.show['col2.importador'] ??
                            true) &&
                        (DspIncomingVisibilityConfig.show['col3.mapa'] ?? true))
                      const SizedBox(width: 12),
                    if (DspIncomingVisibilityConfig.show['col3.mapa'] ?? true)
                      const Expanded(flex: 30, child: Columna3MapaDsp()),
                  ],
                ),
              ),
            ),
          ...
          // Footer
          if (DspIncomingVisibilityConfig.show['footer.toggleTheme'] ?? true)
            AtkFooterBarCommon(
              height: hFooter,
              onModeChanged: (isLight) =>
                  context.read<AppStateManager>().setLight(isLight),
