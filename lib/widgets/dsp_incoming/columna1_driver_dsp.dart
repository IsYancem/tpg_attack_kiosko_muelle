// lib/widgets/dsp_incoming/columna1_driver_dsp.dart
// Autor: Abraham Yance
// Columna 1 — Conductor (versión compacta responsive con soporte de URL de foto)
...
class Columna1DriverDsp extends StatelessWidget {
  const Columna1DriverDsp({super.key});

  @override
  Widget build(BuildContext context) {
    if (!(DspIncomingVisibilityConfig.show['col1.visible'] ?? true)) {
    ...
    final padding = (w * 0.008).clamp(6, 14).toDouble(); // reducido
    final borderRadius = (w * 0.009).clamp(6, 14).toDouble();
    final spacing = (h * 0.01).clamp(4, 10).toDouble(); // menos espacio
    ...
    return Container(
      ...
      child: Column(
        ...
        children: [
          // ─────────────── FOTO DEL CONDUCTOR ───────────────
          if (DspIncomingVisibilityConfig.show['col1.fotoConductor'] ?? true)
            AspectRatio(
          ...
          // ─────────────── CÉDULA ───────────────
          if (DspIncomingVisibilityConfig.show['col1.cedula'] ?? true)
            AtkInfoPanel(
              title: 'CÉDULA',
              value: manager.driverCedula,
              fontSize: w * 0.012, // adaptado y compacto
            ),
