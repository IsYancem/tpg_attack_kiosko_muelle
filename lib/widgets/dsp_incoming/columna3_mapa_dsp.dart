// lib/widgets/dsp_incoming/columna3_mapa_dsp.dart
...
class Columna3MapaDsp extends StatelessWidget {
  const Columna3MapaDsp({super.key});

  @override
  Widget build(BuildContext context) {
    if (!(DspIncomingVisibilityConfig.show['col3.visible'] ?? true)) {
    ...
    return Container(
      ...
      child: Column(
        ...
        children: [
          ...
          // 💬 Mensaje inferior
          if (DspIncomingVisibilityConfig.show['col3.mensajeInferior'] ?? true)
  ...
  Widget _buildMapaImage(BuildContext context, double borderRadius) {
    return Selector<AtkTransactionManager, _MapaDspState>(
      selector: (_, m) => _MapaDspState(m.mapaUrl, m.mapaBytes),
      builder: (context, state, _) {
        ...
        // 2) Si tenemos URL pero no bytes, intentar cargar desde caché
        if (state.url != null &&
            state.url!.isNotEmpty &&
            !state.url!.contains('not_found')) {
          return _MapaDspFromUrl(url: state.url!, borderRadius: borderRadius);
        }
...
class _MapaDspState {
  ...
  _MapaDspState(this.url, this.bytes);

  @override
  bool operator ==(Object other) {
    if (identical(this, other)) return true;
    return other is _MapaDspState && other.url == url && other.bytes == bytes;
  }
...
class _MapaDspFromUrl extends StatefulWidget {
  final String url;
  final double borderRadius;

  const _MapaDspFromUrl({required this.url, required this.borderRadius});

  @override
  State<_MapaDspFromUrl> createState() => _MapaDspFromUrlState();
}

class _MapaDspFromUrlState extends State<_MapaDspFromUrl> {
  Uint8List? _bytes;
  bool _loading = true;
  bool _error = false;
  ...
  @override
  void didUpdateWidget(_MapaDspFromUrl oldWidget) {
