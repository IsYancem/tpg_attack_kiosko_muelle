...
class Columna2ImportadorDspCs extends StatelessWidget {
  const Columna2ImportadorDspCs({super.key});

  @override
  Widget build(BuildContext context) {
    if (!(DspCsIncomingVisibilityConfig.show['col2.visible'] ?? true)) {
    ...
    return Container(
      ...
      child: Column(
        ...
        children: [
          // ⚖️ Peso (más compacto)
          if (DspCsIncomingVisibilityConfig.show['col2.peso'] ?? true)
            Container(
              height: pesoHeight,
              decoration: BoxDecoration(
              ...
              child: Row(
                ...
                children: [
                  if (DspCsIncomingVisibilityConfig.show['col2.peso.icono'] ??
                      true)
                    Icon(
                      Icons.scale_rounded,
                      size: iconSize,
                  ...
                  if (DspCsIncomingVisibilityConfig.show['col2.peso.unidad'] ??
                      true)
                    SizedBox(width: w * 0.01),
                  if (DspCsIncomingVisibilityConfig.show['col2.peso.unidad'] ??
                      true)
          ...
          // 🔷 Encabezado
          if (DspCsIncomingVisibilityConfig.show['col2.encabezado'] ?? true)
            Center(
              child: Text(
          ...
          if (DspCsIncomingVisibilityConfig.show['col2.importador'] ?? true)
            AtkInfoPanel(
              title: 'IMPORTADOR',
              value: manager.importador,
              fontSize: infoFontSize,
            ),
          if (DspCsIncomingVisibilityConfig.show['col2.dres'] ?? true)
            AtkInfoPanel(
              title: 'DRES',
              value: manager.dres,
              fontSize: infoFontSize,
            ),
          if (DspCsIncomingVisibilityConfig.show['col2.turno'] ?? true)
            AtkInfoPanel(
              title: 'TURNO',
              value: manager.turno,
              fontSize: infoFontSize,
